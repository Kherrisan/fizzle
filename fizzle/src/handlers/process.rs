                    ...
                    match f {
                        ...
                        Some(f) => {
                            return Outcome::RunTask(
                                Task::ForkHandler(ForkHandlerTask(f)),
                                YieldUntil::None,
                            )
                        }
                    }
...
pub struct ForkHandlerTask(unsafe extern "C" fn());

impl ForkHandlerTask {
        ...
        match self.state {
            ProcessWaitState::Start => match self.wait_type {
                ...
                WaitType::Pid(child_pid) => {
                    ...
                    if matches!(state.local.process_info.borrow().signal_handlers[libc::SIGCHLD as usize - 1], SigDisposition::Ignore)
                    {
