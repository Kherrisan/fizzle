...
hook_macros::hook! {
    unsafe fn fdopen(
        ...
        let None = state.local.file_objs.insert(file_id, FileObject { descriptor_id, buf: Buffer::new() }) else {
...
hook_macros::hook! {
    unsafe fn fopen(
        ...
        let None = state.local.file_objs.insert(FilePtr::from(file), FileObject { descriptor_id, buf: Buffer::new() }) else {
...
hook_macros::hook! {
    unsafe fn fclose(
        ...
        /*
        let file_id = FilePtr::from(stream);
        match state.local.file_objs.remove(&file_id) {
            Some(fd) => {
                let Some(_) = state.local.fds.remove(fd.descriptor_id) else {
            ...
            None => {
                ...
                if ret == 0 {
                    if let Some(fd) = state.local.passthrough_file_objs.remove(&file_id) {
                        let Some(_) = state.local.fds.remove(fd) else {
